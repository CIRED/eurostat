...
def get_data(code, flags=False, **kwargs):
    ...
    __, provider, __, dsd_last_update = __get_info__(code)

    if cache:
        cached_get_data = cache(__get_data__, ignore=["verbose", "provider"], verbose=verbose)
        alldata = cached_get_data(code, flags, filter_pars, verbose, provider, dsd_last_update)
    else:
        alldata = __get_data__(code, flags, filter_pars, verbose, provider, dsd_last_update)
...
def __get_data__(code, flags, filter_pars, verbose, provider, dsd_last_update):
    if filter_pars == dict():
        filt = "?"
    else:
        start = ""
